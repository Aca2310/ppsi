...
class Administrator extends CI_Controller
{
    ...
    public function pengaduan()
    {
        if ($this->session->userdata('role') != 'admin') {
            ...
            $data['user'] = $this->Users_model->dataAdmin();
            $data['pengaduan'] = $this->Pengaduan_model->viewPengaduanAll();
            $data['pengaduan'] = $this->Pengaduan_model->getUnverifiedPengaduan();
    ...
    public function verifikasi($id_pengaduan)
    {
        $this->Pengaduan_model->verifikasiPengaduan($id_pengaduan);
    ...
    {
        $this->Admin_model->deleteUserAdmin($id_admin);
    ...
    public function addMahasiswa()
    {
        ...
        $validation->set_rules(
            'nim',
            'Nim',
            'required|trim|numeric|min_length[10]|is_unique[mahasiswa.nim]',
            [
                'required' => 'nim tidak boleh kosong!',
                'numeric' => 'nim tidak valid!',
                'min_length' => 'nim tidak terlalu pendek',
                'is_unique' => 'nim tidak ada yang sama'
        ...
        $validation->set_rules(
            'email',
            'Email',
            'required|trim|valid_email|is_unique[mahasiswa.email]',
            [
                'required' => "Email tidak boleh kosong!",
                'valid_email' => "Email tidak valid!",
                'is_unique' => "Email sudah terdaftar, silahkan login!"
        ...
        if ($validation->run()) {
            ...
            $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Akun sudah terdaftar, silahkan Login!</div>');
            redirect('administrator/addMahasiswa');
        } else {

            $data['users'] = $this->Users_model->dataUsers();
            $data['user'] = $this->Users_model->dataMahasiswaRow();

            $data['title'] = "Daftarkan Mahasiswa Baru";
            $this->load->view('_partials/head', $data);
            $this->load->view('_partials/sidebar', $data);
            $this->load->view('_partials/topbar', $data);
            $this->load->view('administrator/add_mahasiswa', $data);
            $this->load->view('_partials/footer');
        }
    }
}
